...
set_password(LUser, LServer, Password) ->
    Username = ejabberd_odbc:escape(LUser),
    {ok, PreparedPass} = prepare_password(LServer, Password),
...
prepare_scrammed_password(Iterations, Password) when is_integer(Iterations) ->
    ...
    {ok, PassDetailsEscaped}.

-spec prepare_password(Server :: ejabberd:server(), Password :: binary()) -> {ok, binary()}.
prepare_password(Server, Password) ->
    case scram:enabled(Server) of
        true ->
            prepare_scrammed_password(scram:iterations(Server), Password);
        _ ->
            {ok, ejabberd_odbc:escape(Password)}
    end.
